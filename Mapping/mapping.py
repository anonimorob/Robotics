...
class Mapping:
    ...
    def update_map(self, grid_map, pose, scan):
        ...
        abs_robot = pose.pose.position.x - origin.position.x 
        ord_robot = pose.pose.position.y - origin.position.y 
        ...
        for j in range(len(scan.ranges)):
            ...
            if scan.ranges[j] > scan.range_min and scan.ranges[j] < scan.range_max:
                pos = scan.angle_min + j* scan.angle_increment
                x_rob = int(((scan.ranges[j] * cos(scan.angle_min + j* scan.angle_increment + robot_yaw)+abs_robot)/ resolution)) 
                y_rob = int(((scan.ranges[j] * sin(scan.angle_min + j* scan.angle_increment + robot_yaw) + ord_robot) / resolution))
                
                obst.append((x_rob, y_rob))
                xi.append(x_rob)
                yi.append(y_rob)
                
                empty_cells = self.raytrace([int((pose.pose.position.x - origin.position.x) / resolution), int((pose.pose.position.y - origin.position.y)/ resolution)], [x_rob, y_rob])
        ...
        for i in obst:
            (ox, oy) = i
            self.add_to_map(grid_map, ox, oy, self.occupied_space)
